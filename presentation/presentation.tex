...
% Cups are nodes in a graph, moves restricted based on graph structure.
...
\begin{frame}[t]{Oblivious Filler Lower Bound}
  ...
  \includegraphics[width=0.3\linewidth]{greedyLike/notallowed.eps}
  \includegraphics[width=0.3\linewidth]{greedyLike/allowed.eps}
  \includegraphics[width=0.3\linewidth]{greedyLike/alsoallowed.eps}
...
\begin{frame}[t]{Amplification Lemma Proof Sketch}
  ...
  \begin{overprint}
    ...
    \onslide<2> Will try to apply $f$ to $B$ while placing $1$ fill in each cup in $A$. 
    \onslide<3> If the emptier doesn't empty from each cup in $A$ (we say it \defn{neglects} $A$) then the average fill of $A$ rises!
    \onslide<4> If emptier doesn't neglect $A$ filler can apply $f$ to $B$
    \onslide<5> this gets a cup with high fill in $B$: swap it into $A$
    \onslide<6> Note: swaps increase average fill of $A$, decrease average fill of $B$.
    \onslide<7> Apply $f$ to $B$ again
    \onslide<8> If emptier doesn't neglect $A$ we have a high cup in $B$ again
    \onslide<9> Swap this cup into $A$.
    \onslide<10> After repeating this process for
    ...
    \onslide<12> $$f'(n) \ge (1-\delta)\sum_{\ell=0}^L f((1-\delta)\delta^\ell n)$$
  \end{overprint}
\end{frame}
