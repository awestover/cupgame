...
\begin{frame}[t]{What is the Cup Game?}
  ...
  each round:
  \begin{itemize}
    \item \defn{filler} distributes $p$ units of water among the cups \\(with at most $1$ unit to any particular cup)
    \item \defn{emptier} chooses $p$ cups and removes (at most) $1$ unit of water from each
  \end{itemize}
  \end{definition}
  ...
  \begin{itemize}
    ...
    \item At each time step:
      \begin{itemize}
        ...
        \item allocate processors to work on tasks (\emph{emptier})
      \end{itemize}
\end{itemize}
\end{frame}
...
\begin{frame}[t]{Our Variant of the Cup Game}
  \begin{definition}
    ...
    Resources are variable\\i.e. filler can change $p$
  \end{definition}
...
\begin{frame}[t]{Adaptive Filler Lower Bound}
  Let $\epsilon > 0$ be any constant. \\By repeated amplification using $\delta=\Theta(1)$ we get: 
  \begin{theorem}
...
\begin{frame}[t]{Oblivious Filler Lower Bound}
  \begin{definition}
    ...
    Can't observe the emptier's moves 
  \end{definition}
...
\begin{frame}[t]{Oblivious Filler Lower Bound}
  \begin{definition}
    ...
    Let $x,y$ be cups. If $\text{fill}(x) > \text{fill}(y) + \Delta$ then a $\Delta$-greedy-like emptier empties from $y$ \emph{only if} it also empties from $x$.
  \end{definition}

  \vspace{0.5cm}
  Oblivious filler can achieve backlog $\Omega(n^{1-\epsilon})$ for $\epsilon >0 $ constant in running time
  $2^{\polylog(n)}$ against $\Delta$-greedy-like emptiers for
  $\Delta \le O(1)$ with probability at least $1-2^{-\polylog(n)}$.

  \vspace{0.5cm}
  Same lower bound as an adaptive filler in quasi-polynomial length games!
\end{frame}
...
\begin{frame}[t]{Flattening}
 ...
 \vspace{1cm}
 \begin{proposition} 
   An oblivious filler can get a $2(2+\Delta)$-flat configuration from an
...
\begin{frame}[t]{Upper Bound Proof Sketch}
  Want to show: $\mu_k(S_t)  \le 2n-k.$ for all $k\in [n], t \in \mathbb{N}$\\
  Define $A, B, C$. Consider extremal fill configuration:
