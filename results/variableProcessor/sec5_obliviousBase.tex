...
We call a cup configuration \defn{$M$-flat} if every cup has fill
at most $M$ different from the average fill.
We say an emptier is \defn{$\Delta$-greedy-like}
...
% note: may as well take $\Delta' = \ceil{\Delta} \in \mathbb{N}$
...
\begin{proof}
  ...
  \defn{non-anchor} set, and $A'$ as the \defn{trash can} set.
...
\randalg to $D$ is \defn{promising} if it achieves backlog
...
\todo{for now we ignore integer issues.}
\todo{for now we ignore the fact that the emptier could be not
nice and skip it's turn or empty from the trash can set :( .}
...
\begin{clm}
  ...
  -h/2$. \todo{or something like that}
\end{clm}
\begin{proof}
  \todo{this argument doesn't work}
