...
\end{theorem}
\begin{proof}
  Take constant $\epsilon \in (0,1/2)$. Let $c, \delta$ be parameters, with $c\in
  (0,1), 0 < \delta \ll 1/2$ -- these will depend on $\epsilon$.
...
0$ and constant $\delta$ satisfying \eqref{eqn:validdeltas}.
  ...
  Then we recursively construct $\alg_{f_{i+1}}$ as the
  amplification of $\alg_{f_{i}}$. 
  ...
  $$f_{i+1}(k) \ge (1-\delta)\sum_{\ell=0}^1 f_i(k\delta^\ell(1-\delta))\ge ck^{1-\epsilon},$$
  as desired. 
  \end{proof}
