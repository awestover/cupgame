...
\begin{proof}
  ...
  \begin{itemize}
    \item \textbf{Step 1:} 
      ...
      have $\fil(c) \ge -h/8$, then we can, with constant probability, achieve
      a cup with fill $h$ in $B$ -- which we will then swap to $A$ -- by
      playing a single-processor cup game on a constant-size subset of $B$. On
      the other hand, if at least half of the cups in $B$ have fill less than
      $-h/8$, then the expectation of positive tilt is very high, so choosing 
      an appropriate number of random cups to swap into $A$ guarantees that we
      have the required positive tilt with the required probability. Since the
      filler is oblivious it does not know which of the cases it is in. Thus
      the filler employs a hybrid of the strategies, switching between them
...
$h'\ge 1/2$ at each step (relative to the average fills); doing so yields backlog
$\Omega(\log n)$.
...
We set apart a constant fraction $\alpha n= n/1000$ of the cups in $A$, which we
call the \defn{storage block}. Let $\gamma = (\lg\lg n)^{1/3}$; we will store between
$\gamma -\gamma/2$ and $\gamma + \gamma/2$ sets of $\beta = \alpha n / (\gamma +
...
With probability $\frac{2\gamma}{n/2}$ the swapping-processes consists of what
we term a \defn{storing-operation}; in a storing-operation the filler takes a 
random subset of $\beta$ cups from $B$ and swaps it into the storage block.
...
which is better than exponentially small in $n$, a much better than the
probability of success that we need.
...
\begin{clm} \label{clm:reg} 
  ...
  $h\le O(1)$). In Case 1, with probability at least $1-e^{-n/8}$, we
  achieve fill at least $h$ in at least $nq/8$ of the cups in $A$ (i.e. a
  constant fraction of the cups in $A$). In particular, this implies that we
  achieve positive tilt $hnq/8$ in $A$.
\end{clm}
\begin{proof}
  ...
  \ge -h$. Note that by assumption there are at least $n/2 - 3/2 \gamma$ such rounds.
  ...
  Clearly $$\E\left[\sum_{i=1}^{n/4} X_i\right] = qn/4.$$ Note that we do not
  use all the $X_i$; we know there must be at least $n/2 - 3/2 \gamma$
  swapping-processes that do not consist of a storing-operation, but only use
  $n/4$ of the $X_i$. We make this choice because the particular constants that
  ...
  $$P\left(\sum_{i=1}^{n/4} X_i\le nq/8\right) \le e^{-n/8}.$$ That is, the
...
\begin{clm}
  ...
  tilt $n h\alpha/48 \ge Omega(n)$ in the anchor set.
\end{clm}
...
\begin{proof}
  ...
  Let $Y_1,Y_2,\ldots, Y_\beta$ be the positive tilts of a randomly selected subset of $\beta$
  cups from $B$, with all subsets equally likely, where $\beta \ge \Omega(n/\lg\lg n)$. 
  Let $X$ be a randomly chosen cup from $B$, with all cups equally likely.
  Note that for any $i$, $$\E[Y_i] = \E[\tilt(X)] = \frac{1}{2}\E[|\fil(X)|] \ge h/2$$ 
  because we are give that there is a set of at least $n/4$ cups with average
  fill more negative than $-h$, which implies that there must be positive tilt
  at least $nh/4$ in some set of cups to offset the negative mass. 
  {\color{red} We're assuming that $\mu(B) = 0$ here; we need to prove
  that this is WLOG by Hoeffding}
  ...
  Thus we have, 
  \begin{align*}
  P\left(\frac{1}{\beta} \sum_{i=1}^{\beta} (Y_i - \E[Y_i]) \le -h/4 \right) \le
  \exp\left(-\frac{2\beta(h/4)^2}{(h\sqrt{n}/\lg\lg n)^2}\right).
  \end{align*}
  Simplifying this gives,
  $$P\left(\frac{1}{\beta}\sum_{i=1}^{\beta} Y_i \le h/4\right) \le \frac{1}{\polylog(n)}.$$
  ...
  Then a Chernof bound gives that
  ...
  $(1-1/\polylog(n))(1-1/\polylog(n)) = 1-1/\polylog(n)$ we achieve positive
  tilt at least $\beta h/4$ in at least $\gamma/8$ sets of $\beta$ cups. In
  total, this means that means we have positive tilt $\beta\gamma h/32 =
  \frac{\alpha n}{(3/2)\gamma}\gamma h/32 = n\alpha h / 48 \ge \Omega(n)$ in the anchor set.
