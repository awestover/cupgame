...
If $f(n) \ge (1-\delta)f(\floor{(1-\delta)n}) + f(\ceil{\delta n})$ then the algorithm simply does $\alg_f$, which achieves the desired backlog in the desired running time. In the rest of the proof, we assume that $f(n) \le (1-\delta)f(\floor{(1-\delta)n}) + f(\ceil{\delta n})$.
...
\begin{proof}
  ...
  on $n_0$ cups (e.g., $n_0 = 1000$ works). Let $\alg_{f_0}$ by the filling strategy described in \cref{prop:adaptiveBase}, where $f_0(k)
  ...
  In particular, we we choose $c$ small enough such that 
  $c n_0^{1-\epsilon} -1 \le 0$, which implies \eqref{eqn:fikinduction} holds for $k \in [n_0]$ by monotonicity of $ck^{1-\epsilon}-1$; we also choose $\delta$ small enough so that $g_0 \ge n_0$ and we choose $c$ small enough such that $c g_0^{1-\epsilon} -1 \le f_0(g_0) = 1$, so that by monotonicity of $ck^{1-\epsilon} -1$ we have \eqref{eqn:fikinduction} for $k \in [n_0, g_0]$. \footnote{Note that for the base case it is important that $\epsilon$ and $\delta$ are constants, that way $c$ is also a constant.}
  ...
  \begin{align*}
    ...
    &\ge ck^{1-\epsilon} \cdot\left(1-(2-\epsilon)\delta + \delta^{1-\epsilon} - (1/2)\delta^{1-\epsilon} \right)-1\\ 
  ...
  \begin{enumerate}
    ...
    \item Our inductive hypothesis; note that $\floor{k\cdot (1-\delta)}$ is small enough that we can use it in $f_i$
    ...
    \item $(1-\delta)^{2-\epsilon} \ge 1-(2-\epsilon)\delta$ by inspection of the first three terms of the Taylor Series of $(1-\delta)^{2-\epsilon}$
    \item $-2(1-\delta) \ge -1$ because $\delta < 1/2$
    \item $1/k^{1-\epsilon} \le 1/g_0^{1-\epsilon} \le \delta^{1-\epsilon}/2$  % CHECK IF ITS le or ge
    \item Combine $\delta^{1-\epsilon}$ terms
  \end{enumerate}
  % make all equations of the form f_{i+1}(k) \ge 
  % Since, blank, we have next equation.

  Because $\delta^{1-\epsilon}$ dominates $\delta$ for sufficiently small $\delta$, there is a choice of constant $\delta$ such that $$f_{i+1}(k) \ge ck^{1-\epsilon}-1,$$
  completing the proof. We remark that the choices of $c, \delta$ are the same for every $i$ in the inductive proof, and depend only on $\epsilon$. 
  \end{proof}
  ...
  $$g_i' = \frac{3-\delta}{(1-\delta)^i},$$
  which clearly exhibits exponential growth. 
  In particular, $g_{(\log_{\frac{1}{1-\delta}} n)}'$, and hence also
  $g_{(\log_{\frac{1}{1-\delta}} n)}$, is at least $n$, as desired.
