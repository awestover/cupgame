...
\begin{proof}
  ...
  removes a cup uniformly randomly from the active set. This
  continues until only a single cup remains in the active set. 
  Consider the probability that this cup has never been emptied
  ...
  In this case, this cup will have gained fill $\sum_{i=2}^n 1/i$
  as claimed. The chosen cup started with fill at
  least $-R$, and hence now has fill at least $-R+ \sum_{i=2}^n 1/i$. 
...
that we can achieve a constant number of unkown cups with high
fill; using this lemma and exploting the greedy-like nature of
...
swapping-process. Let $A_0$ be the set of cups that have been
swapped into the anchor set (note that $A_0$ changes over the
course of the swapping-processes). Before each application of
\randalg the filler flattens $B \cup A\setminus A_0$, and then
separately also flattens $B$. 
...
\ceil{e^{2h+1}}$). Note that on any successful
