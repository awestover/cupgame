...
\begin{proof}
  ...
  The filler uses the following procedure,
  ...
  process is finished. So long as $B$ has average fill at least $-h\cdot
  ...
  The filler applies $f$ to $B$ at most $h n \delta + 1$ times. 
  ...
  more than $h$, so the desired average fill is achieved in the anchor set.
  ...
  in some non-anchor cup, and swaps it with the least full cup in
  the anchor set. Note that by swapping this cup with the least
  full cup of the anchor set we guarantee that the swap does not
  decrease the average fill of $A$, so long as the process is not
  complete yet (i.e. so long as $A$ has average fill below $h$).

  The filler then recurses, and achieves a set of $n\delta^{\ell}$
  cups with average fill increased by
  $(1-\delta)f(n\delta^\ell(1-\delta))$ on the $\ell$-th level of
  recursion (we $0$-index the levels of recursion). Summing for
  $L+1$ levels of recursion, we get backlog 
  $$(1-\delta)\sum_{\ell=0}^L f(n\delta^\ell(1-\delta))$$ 
  as desired.

  Now we analyze the running time of $f'$.
  Note that on the first level of recursion $f'$
  called $f$ on a set of size $(1-\delta)n$ as many as $hn\delta +1$ times. 
  ...
  It turns out not to harm the running time asymptotically to
  take a simpler looser bound obtained by bounding the terms in
  the sum by the first term in the sum:
