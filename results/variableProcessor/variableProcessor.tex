...
\begin{proof}
  ...
  Fix time $t \ge 1$. We assume that $\mu_{S_t}(S_t([k])) \le n-k$ and show
...
OK, but what if $\mu_{S_t}(C) \ge \mu_{S_t}(B\cup C) - b/c$.
...
Now, consider $\mu_{S_t}(C) \le \mu_{S_t}(B\cup C) - b/c$.
We have
...
Note that $\mu_{S_t}(B\cup C) \le \mu_{S_t}(S_t(a+[c]))$.
Thus, 
$$m_{S_{t+1}}(S_{t+1}([k])) \le m_{S_t}(A) + m_{S_t}(S_t(a+[c])).$$
