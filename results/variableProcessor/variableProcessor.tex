...
\end{proposition}
We remark that \cref{prop:obliviousBase} plays a role in the
...
\begin{proof}[Proof of \cref{prop:obliviousBase}]
  The filler starts by flattening the cups, using the flattening procedure
  detailed in \cref{prop:greedylikeisflat}. 

  Let $A$, the \defn{anchor} set, be an arbitrary subset
  of $n/32$ cups and let $B$, the \defn{non-anchor} set, consist of the rest of the cups ($|B| = n\cdot 31/32$). Let $h = (16\Delta + 16)H$. Note that
  the average fills of $A$ and $B$ both must start as at least $-R_\Delta$ due to the flattening.

  The filler sets $p=|A|+1$. We begin by outlining the filler's strategy: \\
  ...
  of $nc$ \textit{known} cups to fill $H$. The emptier, being
  greedy-like, must first empty from the cups with fill $h$ before
  emptying from the cups that the filler is attempting to get fill $H$ in.

To achieve Step 1 the filler performs a series of $|A|$
\defn{swapping-processes}, which are procedures that the filler
uses to get a new cup---which will sometimes have high fill---in
$A$. A swapping-process is composed of a substructure, repeated
many times, which we call a \defn{round-block}; a round-block is
a set of rounds. A swapping-process will consists of $|A|\cdot
c_\Delta$ round-blocks ($c_\Delta = \Theta(1)$ a function of
$\Delta$ to be specified). At the beginning of each
swapping-process the filler chooses a round-block $j$ uniformly
at random from $[|A|\cdot c_\Delta]$, on which it will swap some cup in $B$ and some cup in $A$. 

For each round-block $i\in [|A|\cdot c_\Delta]$, the filler
selects a random subset $D_i\subset B$ of the non-anchor cups and
plays a single processor cup game on $D_i$. In this
single-processor cup game the filler essentially employs the
classic adaptive strategy given in \todo{cite some part of someone's paper} for achieving backlog $\Omega(\log |D_i|)$ on a set of $|D_i|$ cups, with the slight modification that the filler is guessing randomly what the emptier's actions are; we will show that the fill of $\Omega(\log |D_i|)$ is achieved with constant probability. In each step of the single-processor cup game on $D_i$ the filler also places $1$ unit of fill in each cup in the anchor set---this means that the total number of processors in each step is $|A| + 1$. 

% probability: 1/|D_i|!, backlog: \log D_i

On most round-blocks---all but the $j$-th---the filler does
nothing with the results of the single processor cup game.
However, on the $j$-th round-block the filler swaps a cup which
has constant probability of having fill at least $\Omega(\log
|D_i|)$ into the anchor set. Let $A_0$ be the set of cups that
have been swapped into the anchor set (note that $A_0$ changes over the course of the swapping-process). At the end of each
round-block the filler flattens $B \cup A\setminus A_0$, and then separately
also flattens $B$. Note that this will not affect the running
time beyond a multiplicative factor (of e.g. $6$). 

We remark that the construction of a swapping-process is very similar to the
construction in \cref{lem:adaptiveAmplification}, e.g. both
consist of anchoring a set of cups and then playing many cup
games on the non-anchor cups and occasionally swapping a
non-anchor cup and an anchor cup. However, there are two major
differences in the construction for this proof. First, in this
proof we, with exponentially good probability, get many cups with high fill; in the other 
proof all the fill could be concentrated in a few anchor cups.
Second, in this proof we cannot terminate the algorithm upon
achieving the desired backlog, because we can't know when we
achieve the desired backlog. The oblivious filler's disadvantage compared to an adaptive filler prevents the strategy of this proof from working in
general, but against a greedy-like emptier an oblivious filler is able to over come its disadvantage.

Now we show that the Step 1 succeeds with exponentially good probability.
...
\begin{proof}
  ...
  Say the emptier \defn{neglects} the anchor set during a
  round-block if on at least one round of the round-block the
  emptier does not empty from every cup in the anchor set. By
  playing the single-processor cup game for many round-blocks
  with only one round-block when the filler actually swaps a cup
  into the anchor set, the filler prevents the emptier from
  neglecting the anchor set too often.

  On each round-block the filler chooses an arbitrary subset $D_i
  \subset B$ of $\lceil e^{2h+1} \rceil = |D_i|$ cups. If the emptier
  does not neglect the anchor set on round-block $i$ then the
  filler plays a legitimate single-processor cup game on $|D_i|$
  cups. The filler maintains an \defn{active-set} of cups, which
  is a subset of $D_i$, and initialized to be $D_i$. On each
  round of the round-block the filler distributes $1$ unit of
  fill equally among all cups in the active set. Then the emptier
  removes fill from some cup in $B$, possibly $D_i$. The filler
  chooses a random cup to remove from the active set. The
  probability that the cup the emptier emptied from is not in the
  active set after a random cup is removed from the active set by
  the filler is at least constant. By the end of the round-block
  the active-set will consist of a single cup. Let $d =
  \sum_{i=2}^{|D_i|} 1/i$. With constant probability, in
  particular probability at least $q_0 = 1/|D_i|!$ this cup has
  gained fill at least $$d \ge \ln \lceil e^{2h+1} \rceil -1 \ge
  2h.$$

  Consider what this cup's fill started as at the beginning of
  the round-block. By the flattening it was within $R_\Delta$ of
  $\mu(B)$. There are two ways for $\mu(B)$ to change: it can
  ...
  $$\mu(B) \ge -\frac{1}{30}(4h + R_\Delta + \Delta) \ge -h/4.$$
  On the other hand, imagine that after some swap we have  $\mu(A) \ge \mu(B) + d + R_\Delta +
  \Delta$ (note that this must happen due to swaps, not neglect).
  ...
  \begin{equation}
    ...
    \mu(B) \ge -h/2,
  \end{equation}
  and thus a cup that gains fill $d\ge 2h$ in $B$ has fill at
  least $(3/2)h > h$, as desired.
...
Step 2 is easily achieved by setting $p=nc$ and uniformly
distributing the fillers fill among an arbitrarily chosen set
$S\subset B$ of $nc$ cups. Because the emptier is greedy-like, it
must focus on the cups which must exist in $A$ with large
positive fill until the cups in $S$ have sufficiently high fill.
In particular, the fills of the cups in $S$ must start as at
least $-h/2$ by \eqref{eq:Bisntsobad}. After removing
