...
\begin{proof}[Proof of Amplification Lemma]
  ...
  cups with fills at least $\mu(B) + f(|B|)$ and swap these into
  $A$.\\
  \textbf{Step 2:} Use $\alg_f$ once on $A$ to obtain some cup
  with fill $\mu(A)+f(|A|)$, where $\mu(A)\ge h$ due to Step 1.\\
  ...
  prevent the filler from applying $\alg_f$ to $B$.
...
\begin{proof}
  ...
  \begin{clm}
    ...
    \begin{equation}
      ...
      k\in\mathbb{N} \text{ satisfying } k\le g_i.
    \label{eqn:fikinduction}
    \end{equation}
  \end{clm}
  \begin{proof}
  We prove \cref{clm:fikinduction} by induction on $i$.
  In the base case, i.e. for $f_0$, \eqref{eqn:fikinduction} can
  be made true by taking $c$ sufficiently small, in particular small enough
  that $f_0(k) \ge ck^{1-\epsilon}$ holds for $k \le
  ...
  \eqref{eqn:fikinduction} holds for $f_{i+1}$. Note the key
  property of $g_i$, that $g_{i+1}\cdot(1-\delta) \le g_i$. 
  Thus we have that for any $k\in\mathbb{N}$ satisfying $k\le g_{i+1}$,
  \begin{align*}
    f_{i+1}(k) &\ge (1-\delta) f_i(\floor{(1-\delta)k}) + f_i(\ceil{\delta k}) \\
    &\ge (1-\delta) c \floor{(1-\delta)k}^{1-\epsilon} +
    c\ceil{\delta k}^{1-\epsilon} - 1\\
    &\ge (1-\delta) c \cdot ((1-\delta)k-1)^{1-\epsilon} + c(\delta k)^{1-\epsilon} \\
    &\ge (1-\delta) c \cdot (((1-\delta)k)^{1-\epsilon}-1) + c(\delta k)^{1-\epsilon} \\
    &= ck^{1-\epsilon} \cdot\left((1-\delta)^{2-\epsilon} +
    \delta^{1-\epsilon} - 1/k^{1-\epsilon} \right)\\
    &\ge ck^{1-\epsilon} \cdot\left(1-(2-\epsilon)\delta +
    \delta^{1-\epsilon} - 1/k^{1-\epsilon} \right)\\
    &\ge ck^{1-\epsilon} \cdot\left(1-(2-\epsilon)\delta +
    \delta^{1-\epsilon} - (1/2)\delta^{1-\epsilon} \right)\\
    &= ck^{1-\epsilon} \cdot\left(1-(2-\epsilon)\delta +
    (1/2)\delta^{1-\epsilon}\right)
  \end{align*}
  ...
  Clearly $g_{i}$ grows exponentially; in particular $$g_i =
  \frac{2/\delta}{(1-\delta)^i}.$$
  Let $i_* = \log_{\frac{1}{1-\delta}} n.$ We have that $g_{i_*}
  \ge n$.
  Note that $\alg_{f_{i_*}}$ achieves backlog $f(n) \ge cn^{1-\epsilon}$ on $n$ cups.
...
\begin{proof}
  ...
  \cref{prop:adaptiveBase} we can achieve backlog $1$ on these
