...
\begin{proof}
  ...
  Because $(1-\delta)^{2-\epsilon} \ge 1-(2-\epsilon)\delta$, a fact called Bernoulli's Identity, we have
  ...
  \end{align*}
  It follows that $\alg_{f_{i_*}}$, recalling that $i_* \le O(\log n)$, has running time
...
$\delta$ to $\Theta(1/n)$. \footnote{Or more precisely, setting $\delta$ in each level of recursion to be $\Theta(1 / n)$, where $n$ is the subproblem size; note in particular that $\delta$ changes between levels of recursion, which was not the case in the proof of \cref{thm:adaptivePoly}.}
...
\begin{proof}
  ...
  algorithm $\alg_{f_0}$ for
  achieving backlog $f_0(k) \ge 1$ on $k\ge n_0$ cups, which is
  possible by \cref{prop:adaptiveBase}. 
  For $i > 0$ we construct $\alg_{f_{i}}$ as the
  amplification of $\alg_{f_{i-1}}$ using the 
  ...
  As $i_* = \Theta(n)$, we have thus shown that $\alg_{f_{i_*}}$ can achieve backlog $\Omega(n)$ on $n$ cups.

    Let $T_i$ be the running time of $\alg_{f_i}$.
...
can achieve backlog $n^{1-\epsilon}$, although only against a
certain class of ``greedy-like" emptiers (a natural class of emptiers).
...
\end{proposition}
We remark that this proposition plays a similar role to
\cref{prop:adaptiveBase}, but is vastly more complicated to prove.
\begin{proof}[Proof of \cref{prop:obliviousBase}]
  ...
  of $nc$ \emph{known} cups to fill $h'$. The emptier, being
  greedy, must first empty from the
  ...
  least $h'$ for a constant number of levels of recursion.
...
has constant probability of having fill at least $\Omega(\log |B|)$
into the anchor set.
...
\begin{proof}
  ...
  $\lceil e^{2h+1} \rceil$ cups. If the emptier does not neglect the anchor set
  ...
  a subset of $D_i$, and initialized to be $D_i$. On each round of the round-block the
  ...
  probability, in particular probability at least $$q_0 =
  1/\lceil e^{2h+1} \rceil!$$ this cup has gained fill at least
  $$\sum_{i=2}^{\lceil e^{2h+1} \rceil} 1/i \ge \ln \lceil
  e^{2h+1} \rceil -1 \ge 2h.$$
  ...
  has fill strictly more than $s = -h/2-R_\Delta-\lceil e^{2h+1}
  \rceil$, as $B$ starts $R_\Delta$-flat and with average fill at
  least $-h/2$, and the swapped cup is emptied from no more than
  $\lceil e^{2h+1} \rceil$ times. The
  fills of cups in $A$ have fill no more than $R_\Delta$ initially. 
  Hence $\mu(B)$ can certainly rise no higher than
  $u_b = \frac{|A|}{|B|}(R_\Delta - s)+R_\Delta$.
  Using this we can bound how far $\mu(A)$ could sink during this
  process. As shown, any cup swapped into $A$ has fill at least $s$.
  We thus have the (very loose) lower bound that $\mu(A)$ can not
  ...
  Let $\mu_0 = -\ell_a+u_b+R_\Delta+\Delta$. The emptier can neglect the anchor set no
  more than $|A|\mu_0$ times, because it is $\Delta$-greedy-like and neglecting
  the anchor set $|A|\mu_0$ times would increase the mass of the anchor set by
  ...
  increased by $\mu_0$ such a cup with have fill at least
  $R_\Delta+\Delta$ higher than $u_b$, the maximum average fill
  of $B$. Hence such a cup must be emptied from.
  ...
  neglect the anchor set more than $|A|\mu_0$ times. Hence, there
  ...
  the emptiers emptying sequence. Thus, overall, there is at least
...
$S\subset B$ of $nc$ cups. The greedy nature of the emptier will
force it to focus on the cups which must exist in $A$ with large
positive fill until the cups in $S$ have sufficiently high fill.
In particular, the fills of the cups in $S$ must start as at
least $\ell_b \ge -h/2$ by \eqref{eq:Bisntsobad}. After removing
from the very full cups for $\lceil h/2+h' \rceil$ rounds the
fills of these new cups are clearly at least $h'$. Note that
throughout this process the emptier cannot have emptied from the
cups in $S$ until they attained fill $h'$ because there would be
$p=nc$ cups at least $\lceil h/2+h'
...
\begin{lemma}[Oblivious Amplification Lemma]
  ...
  Let $\alg_f$ be an oblivious filling strategy that achieves
  ...
  There exists an oblivious filling strategy $\alg_{f'}$ that
  achieves backlog $f'(n)$ satisfying 
...
\end{lemma}
\begin{proof}
The algorithm defaults to using $\alg_f$ on all the cups if doing
so results in greater backlog than the strategy that we will
outline in the rest of the proof; in this case applying $\alg_f$
...
desired running time. We now outline the filler's strategy if this is not the case.
  ...
  \textbf{Step 1:} Using $\alg_f$ repeatedly on $B$, achieve a
  cup with fill $\mu(B) + f(|B|)$ in $B$ and then swap this cup into $A$. \\
  \textbf{Step 2:} Use $\alg_f$ once on $A$ to obtain a cup in
  $A$ with fill $\mu(A) + f(|A|)$.\\
  Note that in order to use $\alg_f$ on subsets of the cups the filler will need to vary $p$.
  ...
  anchor-set while applying $\alg_f$ to $B$.
  ...
  $\alg_f$ to $B$ if it follows the filling strategy $\alg_f$ on
  $B$ while placing $1$ unit of fill in each anchor cup; during a
  swapping-process the filler repeatedly applies $\alg_f$ to $B$,
  ...
  of $\alg_f$ to $B$ is said to be \defn{successful} if $A$ is
  never neglected during the application of $\alg_f$ to $B$. We
  say that a swapping-process is \defn{successful} if the application of
  $\alg_f$ on which the filler swaps a cup into $A$ is a
  successful application of $\alg_f$.
  ...
  swapping-process consist of $n^{\eta}$ applications of $\alg_f$
  ...
  with probability at least $\delta\mu_\Delta/n^\eta$, when
  averaged over the $|A| \ge n\delta$ swapping processes, this swap
  occurs at the end of a successful application of $\alg_f$ to $B$. 

  If an application of $\alg_f$ is successful, then with
  probability at least $1-2^{-\Omega(n)}$ it generates a cup with
  fill $f(|B|) + \mu(B)$ in $B$, because equal resources were put
  into $B$ on each round while $\alg_f$ was used, and the cup
  ...
  probability of an application of $\alg_f$ being successful is
  only $1-1/\poly(n)$ on average, which is not as good as the
  $1-2^{-\Omega(n)}$ that we will guarantee, we will not be able
  to actually assume that every such application of $\alg_f$ is
  ...
  % like this: $$(1-\delta)^4c((1-\delta)n)^{1-\epsilon} + c(\delta
  % n)^{1-\epsilon} \ge cn^{1-\epsilon}(1-(5-\epsilon)\delta +
  % \delta^{1-\epsilon}) \ge cn^{1-\epsilon}$$ for sufficiently
  % small $\delta$.
  ...
  enough applications of $\alg_f$ within each swapping-process.
  Recall that by construction of $\mu_\Delta$ the emptier cannot
  neglect the anchor set on more than $n\delta \mu_\Delta$
  applications of $\alg_f$ to $B$. There are $n^\eta |A| \ge
  n^{\eta+1}\delta$ applications of $\alg_f$ to $B$. 

  Let $X_i$ be the random variable that indicates the event that
  the $i$-th swapping-process was not successful.
  We {\color{red} almost :( } have, for any constant $\phi$,
  ...
  \begin{align*}
  \Pr\left[\left|\frac{1}{|A|}\sum_{i=1}^{|A|}X_i - \frac{\delta\mu_\Delta}{n^{\eta}}\right| \ge 1-2\phi \right] 
  &\le 2e^{-2|A|(1-2\phi)^2} \\
  &\le 2^{-\Omega(n)}.
  \end{align*}
  By appropriately large choice for $\eta \le O(1)$, $$\delta\mu_\Delta /n^\eta
  ...
  Taking a union bound over all applications of $\alg_f$ we have
  that there is exponentially good probability that all
  applications of $\alg_f$ succeeded. Thus, with exponentially
  ...
  To achieve Step 2 the filler simply applies $\alg_f$ to $A$.
  ...
  application of $\alg_f$ the running time may be increased by a
  ...
  Having proved that $\alg_{f'}$ achieves the desired backlog
