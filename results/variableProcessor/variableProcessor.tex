...
\begin{lemma}[The Adaptive Amplification Lemma]\label{lem:adaptiveAmplification}
  ...
  negative fill allowed), for any constant $\delta\in (0,1)$, there exists a strategy for
  achieving \defn{amplified} fill 
  $$f'(n) \ge (1-\delta)\sum_{i= 0}^{\frac{\lg (n(1-\delta))}{\lg (1/\delta)} } f((1-\delta)\delta^in).$$
\end{lemma}
\begin{proof}
  ...
  Let $A$ the \defn{anchor set} be initialized to consist of the $nk$ fullest
  cups, and let $B$ the \defn{non-anchor set} be initialized to consist of the
  rest of the cups (so $|B| = (1-\delta)n$).
  Let $h_l = (1-\delta)f(n(1-\delta)\delta^{l-1})$; the filler will achieve a set of at least $n_l\cdot \delta =
  n\cdot \delta^l$ cups with average fill at least $h_l$ on the $l$-th level of
  recursion. On the $l$-th level of recursion $|A| = \delta\cdot n_l, |B| = (1-\delta)\cdot n_l$.
  ...
  Thus, if at any point in this process $B$ has average fill lower than $-h_l \cdot \delta/(1-\delta)$,
  then anchor set has average fill at least $h_l$, so the process is
  finished. So long as $B$ has average fill at least $-h_l\cdot \delta/(1-\delta)$ we will
  apply $f$ to $B$.
  ...
  than $h_l$, so the desired average fill is achieved in the anchor set.
  ...
  $$-h_l \cdot \delta/(1-\delta) + f(n_l (1-\delta)) = (1-\delta)f(n_l (1-\delta)) = h_l$$
...
\begin{proof}$ $\\
  ...
  \paragraph{Further Generalizing Our Approach}
  ...
  the $O(\delta^2)$ term is less than $M \delta^2$ for some constant $M$.
  Taking $\delta$ small enough such that $\delta^2 M < \delta$, we have that
  ...
  However, it is not obvious that this backlog is achievable. We now show that
  the equality case of the Amplification Lemma is achieved.

  YAY PASCALS TRIANGLE.
