...
running time. Proposition \ref{prop:expoTimeAlg} also provides an extremal strategy
...
in exponentially long games. Although the running time is very large, it is
...
\begin{proposition}
  \label{prop:expoTimeAlg}
  There is an adaptive filling strategy that achieves backlog $\Omega(n)$ in time $2^{O(n)}$.
\end{proposition}
\begin{proof}
