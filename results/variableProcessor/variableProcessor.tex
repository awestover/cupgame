...
\begin{lemma}
  The greedy emptier maintains the invariant $\mu_{S_t}(S_t([k])) \le n-k$ for all $t\in\mathbb{N}, k \le n$.
...
\begin{proof}
  ...
  Fix any $k < n$. We prove that $\mu_{S_t}(S_t([k]))\le n-k$ for all $t\in\mathbb{N}$ by induction on $t$.
  The Lemma holds trivially for $t=1$: the cups start empty so $\mu_{S_1}(S_1([k])) = 0 \le n-k$.

  Fix a round $t \ge 1$. We assume that $\mu_{S_t}(S_t([k])) \le n-k$ and show
  that $\mu_{S_{t+1}}(S_{t+1}([k])) \le n-k$. 
...
With these definitions made, we proceed to prove the Lemma. We will analyze several cases.
...
\textbf{Conclusion:}\\
Thus, by induction we have the invariant for all rounds $t\in\mathbb{N}$, and
since $k$ was arbitrary we have the invariant for all $k$.
\end{proof}
\clearpage
