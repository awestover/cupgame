...
\end{proposition}
\begin{proof}
  Let the fills of the $2$ cups start as $x, -x$ for some $x\ge
  0$. If $x\ge 1/2$ the algorithm halts. Otherwise, the filling
...
\begin{proof}
  ...
  Because $(1-\delta)^{2-\epsilon} \ge 1-(2-\epsilon)\delta$, a fact called Bernouli's Identity, we have
  ...
  $$T(n) \le n^2\delta \cdot T(\floor{(1-\delta)n}) +
  T(\ceil{\delta n}) \le 2n^2T(\floor{(1-\delta)n}).$$
...
\begin{proof}
  ...
  We claim the following regarding this constuction:
  \begin{clm}
  ...
  Because $f_{i}$ is the amplification of $f_{i-1}$ with $\delta = 1/(i+1)$, we have
  ...
  As $i_* = \Theta(n)$, we have thus shown that $\alg_{f_{i_*}}$ can achieve backolog $\Omega(n)$ on $n$ cups.

  The recurrence for the running running time of $f_{i_*}$ is 
  $$T(n) \le n \cdot n_0T(n-n_0)+O(1).$$
  Clearly $T(n) \le O(n!)$.
...
\begin{lemma}[Oblivious Amplification Lemma]
  ...
  Let $\Delta \le O(1)$, $M, M' \ge R_\Delta$, $q \ge \Omega(1)$. Let $\alg_f$ be an
  oblivious filling strategy that achieves backlog $f(n)$ in the negative-fill
  variable-processor cup game on $n$ cups with probability at least $1-2^{-\Omega(n)}$
  in running time $T(n) \le \poly(n)$ when given a $M$-flat configuration,
  against a $\Delta$-greedy-like emptier.

  Let $0 < \delta \ll 1/2$, $\eta \in \mathbb{N}$, $1/2 \ll
  \phi<1$ be constant parameters. 

  There exists an oblivious filling strategy $\alg_{f'}$ that
  achieves backlog $f'(n)$ satisfying 
  $$f'(n) \ge \phi \cdot (1-\delta) f(\floor{(1-\delta)n}) + f(\ceil{\delta n})$$ 
  and $f'(n) \ge f(n)$ in the negative-fill variable-processor cup game on $n$ cups given a
  $M'$-flat configuration of cups in running time $$T'(n) \le
  O(M') + \delta n^{\eta+1} T(\floor{(1-\delta)n}) +
  T(\ceil{\delta n})$$ against a $\Delta$-greedy-like emptier
  with probability at least $$1-2^{-\Omega(n)}.$$
\end{lemma}
\begin{proof}
The algorithm defaults to using $\alg_f$ if $f(n) \ge
\phi \cdot (1-\delta)f(\floor{(1-\delta)n}) + f(\ceil{\delta n})$; in this
case using $\alg_f$
achieves the desired backlog in the desired running time. In the
rest of the proof, we describe our strategy for achieving backlog
$\phi \cdot (1-\delta)f(\floor{(1-\delta)n}) + f(\ceil{\delta n})$. 
  ...
  least $-R_\Delta$ due to the flattening. Let $h =
  (1-\delta)f(\floor{(1-\delta)n})$.
  ...
  never neglected during the application of $\alg_f$ to $B$.
  ...
  with probability at least $\delta\mu_\Delta/n^\eta$ this swap
  ...
  Now we aim to show that $\mu(A) \ge h$. To do so, it suffices
  to show that the average fill of $B$ decreases by at least
  $-h \cdot \delta/(1-\delta)$.
  The filler swaps $\delta n$ cups into $B$. 
  Consider how $\mu(B \cup A\setminus A_0)$ changes when the a new
  ...
  $$\mu(B) \le -\sum_{i=1}^{|A|} \frac{f(|B|)-R_\Delta}{n-i}.$$
  Now we bound $H_{n-1} - H_{n-|A|-1}$ where $H_i$ is the $i$-th harmonic number.
  \begin{align*}
    H_{n-1} - H_{n-|A|-1}
  &\ge \ln \frac{n-1}{n-|A|-1} - \frac{1}{2(n-|A|-1)}\\
  &\ge \ln \frac{n}{n-|A|} - \frac{1}{n}\\
  &= \ln \frac{n}{n-\ceil{\delta n}} - \frac{1}{n}\\
  &\ge \ln \frac{1}{1-\delta} - \frac{1}{n}\\
  &\ge \delta - \frac{1}{n}.
  \end{align*}
  % this is due to the asymptotic expansion of H_n as 
  % $H_n = \ln n + gamma + 1/(2n) - 1/(12n^2) + 1/(120n^4) - \ldots$
