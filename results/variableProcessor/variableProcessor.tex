...
\begin{lemma}[Oblivious Amplification Lemma]
  ...
  \delta n^{\eta+1} T(\floor{(1-\delta)n}) + T(\ceil{\delta n})$$
...
to all the cups trivially achieves the desried backlog in the
desired running time. We now outline the filler's strategy if this is not the case.
  ...
  only $1-1/\poly(n)$, which is not as good as the
  ...
  Hence we have, 
  $$\mu(A) \ge \frac{(1-\delta)}{\delta}\paren{\delta\phi-\frac{1}{n}}(f(|B|)-R_\Delta)$$
