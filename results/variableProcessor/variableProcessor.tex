...
We set apart a constant fraction $\alpha = n/1000$ of the cups in $A$, which we
call the \defn{storage block}. Let $\gamma = 2\lg\lg n$; we will store between
$\gamma -\gamma/2$ and $\gamma + \gamma/2$ sets of $\beta = \alpha / (\gamma +
...
\gamma/2$ of its expectation $\gamma$ is at least $1-2e^{-n(\lg\lg n)^2}$,
which is much better than the probability of success that we need.
...
\begin{clm}
  ...
  {\color{red} nope this just isnt what we prove. I think its an instant kill if we get this thing going. so really we say, In case 2 with probability at least 1/polylog n we instant kill}
\end{clm}
...
\begin{proof}
  ...
  {\color{red} I think we're assuming that $\mu(B) = 0$ here. We can fix this easily,
  I think, by selecting $X$ is drawn from $A\cup B$, and also $Y$ drawn from
  $A\cup B$}
  ...
  $$P\left(\sum_{i=1}^{\beta} Y_i \le \beta h/4\right) \le \frac{1}{\polylog(n)}.$$
