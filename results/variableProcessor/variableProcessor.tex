...
\begin{proof}$ $\\
  ...
  Now we proceed to show that with the chosen values of $\delta, \epsilon$ we
  ...
  that achieves the desired backlog exists. Then we will provide a constructive
  argument that uses repeated amplification of a base case to
  achieve a function, in finite running time, with the same backlog guarantee
  on $n$ cups.
  
  \paragraph{Existential Argument}
  Let $f^*$ be the supremum over all filling strategies of the backlog
  achievable on $n$ cups. Then $f^*$ satisfies the Amplification Lemma. Assume
  for contradiction that there is some least $n_*$ such that $f^*(n) <
  cn^{1-\epsilon}$, whereas by appropriate choice of $c$ $f^*(n) \ge cn^{1-\epsilon}$
  for $n < n_*$ which is not the empty set. Because $f^*$ satisfies the
  Amplification Lemma we have:
\begin{align*}
  f^*(n_*) & \\
           ...
           &\ge cn_*^{1-\epsilon}
\end{align*}
which is a contradiction. Hence $f^*$ achieves backlog $cn^{1-\epsilon} = \Omega(n)$ for all $n$.

\paragraph{Constructive Argument}
