...
call \defn{$\randalg$}, that will be used as a
...
\begin{proposition}
  ...
  There is an oblivious filling strategy \defn{$\randalg(k)$} with running
  time $k-1$ that, if we condition on the emptier not performing extra
  emptyings, achieves fill at least $\mu_0 -R + d$ in
  a known cup $c$ with probability at least $1/(n-1)!$.
  This filling strategy achieves fill at most $\mu_0 + R + d$ in
  this cup, unconditionally.
...
\begin{proof}
  ...
  $1/(k-1)!$ of attaining a cup with fill at least $\mu_0 -R +d$.
  The filler maintains an \defn{active set}, initialized to being
  an arbitrary subset of $k$ of the cups. Every round the filler distributes $1$ unit of
