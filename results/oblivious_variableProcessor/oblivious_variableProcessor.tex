...
\begin{lemma}[The Oblivious Amplification Lemma]
  ...
  configuration of cups for specified $T \le \poly (n)$. Let $\delta \in (0,1)$
  be a parameter. Let $L \le O(1)$ be an appropriate parameter. Let $c \le
  O(1)$ be an appropriately small ($c\ll 1$) constant. Then, there exists an
  adaptive filling strategy that, with constant probability, given a $T$-flat
  configuration of cups with $T \le \poly(n)$ achieves backlog $$f'(n) \ge
  (1-\delta)\sum_{\ell=0}^L f((1-\delta)(c\delta)^\ell n)$$ in the variable
  processor cup game on $n$ cups in running time $\poly(n)$ against a
  $\Delta$-greedy-like emptier for $\Delta \le O(1)$ a constant known to the
  filler.
\end{lemma}
\begin{proof}
  ...
  \begin{itemize}
    ...
    \item \textbf{Step 2:} Reduce the number of processors to $p=n\delta$ and recurse on $A$.
  \end{itemize}
  ...
  For Step 2 we simply recurse on $A$. This achieves the desired backlog. The
  probability of any layer of recursion failing is bounded above by 
  $$\sum_{\ell=0}^L e^{-nq (c\delta)^\ell}$$
