...
\author{William Kuszmaul\textsuperscript{1}, Alek Westover\textsuperscript{1}}
...
\abstract{
  ...
  settings, dating back to the late 1960's. Kuszmaul established
  that a greedy emptier never lets backlog exceed $O(\log n)$.
  Kuszmaul also gave a construction that achieves backlog 
  $\Omega(\log (n-p))$. For small $p$, e.g. $p< n/2$, it is
  clear that $\Theta(\log n)$ is the optimal backlog, but for
  very large $p$ tight bounds are not known. 

  We give an adaptive filling strategy for the $p$-processor
