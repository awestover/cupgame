...
We now prove that greedy-like emptiers are flattenable. This follows
...
\begin{proof}
  ...
  \begin{proof}
    ...
    If $c$ is not emptied from, i.e. $\fil_{t+1}(c) = \fil_t(c)+1/2$, then
    ...
    On the other hand, if $c$ is emptied from, i.e. $\fil_{t+1}(c) = \fil_t(c) - 1/2$, we consider two cases.
    \begin{itemize}
      \item If $\fil_t(c) \ge u_t-\Delta -1$, then, as $u_{t+1} \le u_t+1/2$, 
        $$\fil_{t+1}(c) \ge u_t-\Delta-1 - 1/2\ge u_{t+1}-\Delta-2.$$
      \item On the other hand, if $\fil_t(c) < u_t-\Delta-1$, then every cup
  ...
  this implies that all cups have fill in $[-2(d+\Delta), 2(2+\Delta)]$.
